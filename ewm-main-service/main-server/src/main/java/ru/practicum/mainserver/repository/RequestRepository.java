...
public interface RequestRepository extends JpaRepository<RequestEntity, Long> {
    int countByEvent_IdAndStatusIn(Long id, Collection<StatusEnum> statuses);
    List<RequestEntity> findByEvent_IdAndEvent_Initiator_Id(Long id, Long id1);

    List<RequestEntity> findByEvent_IdAndRequester_Id(Long id, Long id1);

    List<RequestEntity> findByEvent_Id(Long id);

    long countByEvent_IdAndStatus(Long id, StatusEnum status);

    List<RequestEntity> findByEvent_IdInAndStatus(Collection<Long> ids, StatusEnum status);
